...
const HasilKuis = () => {
  ...
  const { jenisKuis, nextMaterialPath, prevMaterialPath, retryQuizPath } = location.state || {};
...
useEffect(() => {
  if (!mahasiswaId || !jenisKuis) return;

  const fetchNilai = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/nilai/${encodeURIComponent(jenisKuis)}`, {
