...
import questions from "../../../data/kuis/kuis1.json";
...
const Kuis1 = () => {
  ...
  const [halamanAktif, setHalamanAktif] = useState(2);
  ...
  const cekSudahLulus = async () => {
    ...
    try {
      ...
      const resNilai = await axios.get(
        `http://localhost:5000/nilai/terbaru?mahasiswaId=${mahasiswaId}&jenisKuis=Pengenalan Object`,
        {
          headers: {
      ...
      if (nilai >= nilaiKKM) {
        navigate("/hasil-kuis", {
          state: {
            mahasiswaId,
            tokenKelas,
            jenisKuis: "Pengenalan Object",
            nextMaterialPath: "/materi/materiDom",
  ...
  const handleFinishQuiz = async () => {
    ...
    if (score >= kkm && halamanAktif > progress) {
  ...
  return (
    <>
      <KuisHeader title="Kuis Pengenalan Object" durationMinutes={25} />
