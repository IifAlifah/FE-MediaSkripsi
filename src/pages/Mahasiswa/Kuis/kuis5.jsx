...
import questions from "../../../data/kuis/kuis5.json";
...
const Kuis5 = () => {
  ...
  const [halamanAktif, setHalamanAktif] = useState(19); // halaman saat ini
  ...
  const cekSudahLulus = async () => {
    ...
    try {
      ...
      const resNilai = await axios.get(
        `http://localhost:5000/nilai/terbaru?mahasiswaId=${mahasiswaId}&jenisKuis=Event DOM`,
        {
          headers: {
      ...
      if (nilai >= nilaiKKM) {
        navigate("/hasil-kuis", {
         state: {
          ...
          tokenKelas, 
          jenisKuis: "Event DOM",
          nextMaterialPath: "/materi/mengaksesForm-elemenForm",
          prevMaterialPath: "/materi/mengenalEvent",
          retryQuizPath: "/kuis5", 
        },
        });
      }
  ...
  return (
    <>
      <KuisHeader title="Kuis Event Dom" durationMinutes={30} />
