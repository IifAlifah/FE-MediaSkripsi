...
import questions from "../../../data/kuis/kuis3.json";
...
const Kuis3 = () => {
  ...
  const [halamanAktif, setHalamanAktif] = useState(9); // halaman saat ini
  ...
  const cekSudahLulus = async () => {
    ...
    try {
      ...
      const resNilai = await axios.get(
        `http://localhost:5000/nilai/terbaru?mahasiswaId=${mahasiswaId}&jenisKuis=Mengakses Elemen`,
        {
          headers: {
      ...
      if (nilai >= nilaiKKM) {
        navigate("/hasil-kuis", {
          state: {
            mahasiswaId, // Mengambil userId dari localStorage
            tokenKelas, 
            jenisKuis: "Mengakses Elemen",
            nextMaterialPath: "/materi/mengubahKonten",
            prevMaterialPath: "/materi/id",
            retryQuizPath: "/kuis3", // Path untuk mengulang kuis
          },
        });
      }
    } catch (error) {
  ...
  return (
    <>
      <KuisHeader title="Kuis Mengakses Elemen" durationMinutes={30} />
