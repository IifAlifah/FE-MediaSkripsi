...
import questions from "../../../data/kuis/kuis2.json";
...
const Kuis2 = () => {
  ...
  const [halamanAktif, setHalamanAktif] = useState(3);
  ...
  const cekSudahLulus = async () => {
    ...
    try {
      ...
      const resNilai = await axios.get(
        `http://localhost:5000/nilai/terbaru?mahasiswaId=${mahasiswaId}&jenisKuis=Pendahuluan`,
        {
          headers: {
      ...
      if (nilai >= nilaiKKM) {
        navigate("/hasil-kuis", {
          state: {
            mahasiswaId,
            tokenKelas,
            jenisKuis: "Pendahuluan",
            nextMaterialPath: "/materi/id",
  ...
  return (
    <>
      <KuisHeader title="Kuis Pendahuluan" durationMinutes={25} />
